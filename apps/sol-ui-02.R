...
ui <- fluidPage(
  ...
  fluidRow(
    ...
    column(5, verbatimTextOutput("summaryOutput"))
  ),
  fluidRow(
    column(3, numericInput('beta0', withMathJax(), label = "Intercept coef: \\(\\beta_0\\)",
                           min = -50, max = 50, value = 10),
              numericInput("n", label = "Sample size",
                           min = 2, max = 1024, value = 128),
              actionButton("simulate", label = "Simulate")
           ),
    column(3, numericInput('beta1', withMathJax(), label = "Slope coef: \\(\\beta_1\\)",
                           min = -50, max = 50, value = 10),
    ...
    column(3, checkboxInput("showTrueLine", label = "Show True regression line"),
...
server <- function(input, output) {
  ...
  output$scatterplot <- renderPlot({
    ...
    if(input$showTrueLine) {
      p <- p + geom_line(aes(x = x, y = mu), linetype = "dashed")
    }
